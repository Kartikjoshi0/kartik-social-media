...
function Profile({post}) {
  ...
  useEffect(()=>{
    ...
    setIsFollowing(feedData?.followings?.find(item=>item._id===params.user._id))
  },[myProfile,params.userId,feedData]);

  function handleUserFollow(){
    dispatch(followAndUnfollowUser({
      userIdToFollow:params.user?._id
    }))
  }
    ...
    <div className='Profile'>
      <div className="container">
        <div className="left-part">
               ...
               {userProfile?.posts?.map(item => <Post key={post._id} post={post}/>)}
        </div>
        <div className="right-part">
          <div className="profile-card">
            ...
            {!myProfile && <h5 style={{marginTop:'10px'}} onClick={handleUserFollow}
